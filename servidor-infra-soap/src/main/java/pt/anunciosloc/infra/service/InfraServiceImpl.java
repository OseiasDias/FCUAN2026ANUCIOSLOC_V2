...
public String[] listarLocaisPorUtilizador(String email) {
    ...
    @Override
public String[] listarMeusLocais(String email) {
    System.out.println("=== LISTAR MEUS LOCAIS ===");
    System.out.println("Email: " + email);
    ...
    try {
        ...
        if (userId == null) {
            System.out.println("Utilizador nao encontrado: " + email);
            return new String[0];
        }
        
        // Buscar todos os locais criados pelo utilizador
        String sql = "SELECT id, nome, tipo, latitude, longitude, raio, wifi_ssid " +
                     "FROM locais WHERE criado_por = ? ORDER BY id DESC";
        ...
        System.err.println("Erro ao listar meus locais: " + e.getMessage());
        return new String[0];
    }
}
}